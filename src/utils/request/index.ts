...
const axiosHooks: AxiosHooks = {
  requestInterceptorsHook(config) {
    const { isParamsToData, withProgress } = config.requestOptions
    if (withProgress) {
      NProgress.start()
    }
  ...
  async responseInterceptorsHook(response) {
    ...
    switch (code) {
      case RequestCodeEnum.SUCCESS:
        if (show) {
          msg && showNotify({ type: 'danger', message: msg })
        }
        return data
      ...
      case RequestCodeEnum.MERCHANT_EXPIRE:
      case RequestCodeEnum.MERCHANT_DISABLE:
